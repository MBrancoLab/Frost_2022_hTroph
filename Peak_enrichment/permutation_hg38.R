...
##RepeatMasker annotation (see 'RMasker_outToBed.R')
rm.file = '~/genomes/GRCh38/Repeatmasker_hg38_4.0.5_noSimple.bed'
...
real.overlap = tapply(bed.rm$V4,factor(bed.rm$V4,levels=unique(rmask$V4)),length)
...
for (i in 1:n) {
	...
	rand.overlaps[,i] = tapply(bed.rand.rm$V4,factor(bed.rand.rm$V4,levels=unique(rmask$V4)),length)
...
out = data.frame(log.enrich,pval,total=rm.count,real.overlap,random.mean)
